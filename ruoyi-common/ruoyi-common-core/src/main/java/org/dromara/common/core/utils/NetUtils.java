...
// PatternPool提供预编译的正则表达式，避免重复编译，提升性能
import cn.hutool.core.lang.PatternPool;
// Hutool网络工具类（提供IP、端口等基础网络操作）
// NetUtil是Hutool封装的网络操作工具类
...
// 正则表达式工具类（封装正则匹配逻辑）
// RegexUtils是项目封装的正则表达式工具类
import org.dromara.common.core.utils.regex.RegexUtils;

// Java网络编程核心类
import java.net.Inet6Address; // IPv6地址类，表示IPv6地址
import java.net.InetAddress; // IP地址基类，是IPv4和IPv6地址的父类
import java.net.UnknownHostException; // 未知主机异常，当IP地址格式错误时抛出

/**
 * 增强网络相关工具类
 * 继承Hutool的NetUtil，提供IP地址类型判断、内外网判断等增强功能
 * 支持IPv4和IPv6地址的格式验证和分类
 * 常用于IP地址解析、内外网判断、IP格式验证等场景
 *
 * @author 秋辞未寒
 */
...
// 网络工具类，继承Hutool的NetUtil，提供增强的网络操作能力
public class NetUtils extends NetUtil {

    /**
     * 判断是否为IPv6地址
     * 通过InetAddress.getByName解析IP字符串，判断实例类型是否为Inet6Address
     * 支持标准IPv6格式（如2001:0db8:85a3:0000:0000:8a2e:0370:7334）和压缩格式（如2001:db8::1）
     * 使用Java标准库进行解析，准确可靠
     *
     * @param ip IP地址字符串
    ...
    public static boolean isIPv6(String ip) {
        try {
            // 使用InetAddress.getByName()方法解析IP字符串
            // InetAddress是Java网络编程的核心类，支持IPv4和IPv6
            // 如果解析成功且返回Inet6Address实例，则说明是IPv6地址
            // 使用instanceof判断实例类型，是Java多态的典型应用
            // 使用InetAddress.getByName()方法解析IP字符串，判断是否为IPv6地址
            return InetAddress.getByName(ip) instanceof Inet6Address;
        } catch (UnknownHostException e) {
            // 捕获UnknownHostException异常，表示IP地址格式错误
            // 解析失败（格式错误），返回false
            // 记录日志，方便排查问题
            // 捕获UnknownHostException异常，记录日志并返回false
            log.debug("Invalid IPv6 address: {}", ip, e);
            return false;
        }
    }
    ...
    public static boolean isInnerIPv6(String ip) {
        try {
            ...
            if (InetAddress.getByName(ip) instanceof Inet6Address inet6Address) {
                ...
                // 满足任一条件即认为是内网地址
                // 使用Inet6Address的方法判断地址类型，满足任一条件即认为是内网地址
            ...
            // 记录错误日志，方便定位问题
            // 捕获UnknownHostException异常，记录错误日志并抛出IllegalArgumentException
            log.error("Invalid IPv6 address: {}", ip, e);
     ...
     * 正则表达式：^(1\d{2}|2[0-4]\d|25[0-5]|[1-9]\d|[1-9])\.(1\d{2}|2[0-4]\d|25[0-5]|[1-9]\d|\d)\.(1\d{2}|2[0-4]\d|25[0-5]|[1-9]\d|\d)\.(1\d{2}|2[0-4]\d|25[0-5]|[1-9]\d|\d)$
     *
     * @param ip IP地址字符串
    ...
    public static boolean isIPv4(String ip) {
        ...
        // 预编译正则表达式是性能优化的常用手段，避免每次调用都编译正则
        // 调用RegexUtils.isMatch方法，使用预编译的IPv4正则表达式进行匹配
